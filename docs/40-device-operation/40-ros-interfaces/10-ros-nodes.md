...
Камера запускается отдельным systemd-сервисом `camera.service`, а не общим сервисом `ros_nodes.service`. Благодаря этому при сбое ноды камеры перезапускается только камера, остальные ROS-ноды продолжают работать.
